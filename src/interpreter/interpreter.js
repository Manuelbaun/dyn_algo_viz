...
var Interpreter = function (code, opt_initFunc) {
  ...
  var state = new Interpreter.State(this.ast, this.globalScope);
  state.done = false;
  this.stateStack.setLength(0);
  this.stateStack.set(0, state);
...
Interpreter.prototype.appendCode = function (code) {
  var state = this.stateStack.get(0);
...
Interpreter.prototype.step = function () {
  ...
  do {
    ...
    /**
     * Added by Manel Baun:
     * This function will be called, to perform some more expression,
     * like enable breakpoints etc.
     *
     */
    this.onStep && this.onStep(state);
...
Interpreter.prototype.initArray = function (globalObject) {
  ...
  wrapper = function (var_args) {
    const data = Array.prototype.unshift.apply(this.properties, arguments);
  };
  ...
  wrapper = function (var_args) {
      ...
      if (thisInterpreter.isa(value, thisInterpreter.ARRAY)) {
        ...
        for (var j = 0; j < jLength; j++) {
          if (thisInterpreter.hasProperty(value, j)) {
            data[length] = thisInterpreter.getProperty(value, j);
          }
          length++;
        }
      } else {
    ...
    return thisInterpreter.arrayNativeToPseudo(data);
  };
...
Interpreter.prototype["stepArrayExpression"] = function (stack, state, node) {
  ...
  stack.pop();
  stack.getTop().value = state.array_;
};
...
Interpreter.prototype["stepAssignmentExpression"] = function (
  ...
  if (state.doneSetter_) {
    ...
    stack.pop();
    stack.getTop().value = state.setterValue_;
    return;
  }
...
Interpreter.prototype["stepFunctionExpression"] = function (
  ...
  stack.pop();
  stack.getTop().value = this.createFunction(node, state.scope);
};

Interpreter.prototype["stepIdentifier"] = function (stack, state, node) {
  stack.pop();
  if (state.components) {
    stack.getTop().value = [Interpreter.SCOPE_REFERENCE, node["name"]];
    return;
  }
...
Interpreter.prototype["stepMemberExpression"] = function (stack, state, node) {
  ...
  stack.pop();
  if (state.components) {
    stack.getTop().value = [state.object_, propName];
  } else {
...
Interpreter.prototype["stepObjectExpression"] = function (stack, state, node) {
  ...
  stack.pop();
  stack.getTop().value = state.object_;
};
...
Interpreter.prototype["stepThisExpression"] = function (stack, state, node) {
  stack.pop();
  stack.getTop().value = this.getValueFromScope("this");
};
...
Interpreter.prototype["stepUpdateExpression"] = function (stack, state, node) {
  ...
  // Return if no setter function.
  stack.pop();
  stack.getTop().value = returnValue;
};
